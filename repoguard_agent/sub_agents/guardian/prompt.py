...
1. **Forensic Investigation**:
   ...
   - Use GitLab MCP tools (prefixed with `gitlab_`) to investigate.
   - Use `gitlab_list_repository_commits` or `get_commits_in_window_tool` to find commits in the 10-minute window preceding the alert.
   - For each candidate commit, use `gitlab_get_repository_commit_diff` or `get_commit_diff_tool` to inspect changes.
   - Identify the "Root Cause Commit" that plausibly explains the error based on the stack trace.

2. **Provision Remediation**:
   - Use `gitlab_create_repository_branch` or `create_branch_tool` to create a rollback branch (branch name: `emergency/rollback-{SHORT_SHA}`).
   - Revert the offending commit on this branch using `revert_commit_tool`.
   - Create a Draft Merge Request using `gitlab_create_merge_request` or `create_mr_tool` with a descriptive title and body explaining the forensic analysis.
